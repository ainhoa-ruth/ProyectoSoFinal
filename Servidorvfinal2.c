...
void *AtenderCliente (void *socket)
{
	...
	while (terminar ==0)
	{
		...
		case 11:
			...
			p = strtok (NULL, "/");
			char reenvio [1000];
			sprintf(reenvio, "11/%s/%d/%d",nombre,ID,jugada);
			
			j=0;
			while(j < listap.partidas[ID].numParticipantes)
			{
				
				write (listap.partidas[ID].conectados[j].socket,reenvio, strlen(reenvio));
				j++;
			}
			break;
		case 12:
			...
			p = strtok (NULL, "/");
			char reenvio2 [1000];
			sprintf(reenvio2, "12/%s/%d/%d",nombre,ID,jugada2);
			
			j=0;
			while(j < listap.partidas[ID].numParticipantes)
			{
				
				write (listap.partidas[ID].conectados[j].socket,reenvio2, strlen(reenvio2));
				j++;
			}
		...
		case 13:
			...
			if (error != 0)
			{
				...
				printf ("Ha ocurrido un error en el caso 13");
				sprintf(respuesta13,"13/err13");
				write (sock_conn,respuesta13,strlen(respuesta13));
			}
			else
			{
				sprintf(respuesta13,"13/%s",nombre);
				write (sock_conn,respuesta13,strlen(respuesta13));
			}	
			break;
		default:
